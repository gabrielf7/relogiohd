...
function CarregarRelogio() {
  ...
  // ↓ Função ↓
    ...
    function formatoDoFusoHorario(tamanho="") {
      ...
      return dataAtual().toLocaleDateString("pt-br", nomeDoFusoHorario).slice(4);
    }
    ...
    function InformarDataDeHJ() {
      let infoData = (op) => DataHorarioHJ("datahj").toLocaleString("pt-br", op);
    ...
    function statusDoDia() {
      if((horaAtual() == 5) || (horaAtual() == 12) ||
        (horaAtual() == 17) || (horaAtual() == 23)
      ){
        if((minutoAtual() == 59) && (segundoAtual() == 59)){
    ...
    function TempoDoRelogio() {
      ...
      const segundos = (segundoAtual() / 60);
      const minutos = ((segundos + minutoAtual()) / 60);
      const horas = ((minutos + horaAtual()) / 12);
